...
def add_patient(request):
    ...
    if request.method == 'POST':
        ...
        # Get tenant from request (assuming tenant middleware sets this)
        tenant = getattr(request, 'tenant', None)
        if not tenant:
            # Fallback: try to get from session or use default
        ...
        if Patient.objects.filter(tenant=tenant, mrn=mrn).exists():
            messages.error(request, _('A patient with this MRN already exists in your system'))
            return redirect('patients:add_patient')
        ...
        try:
            patient = Patient.objects.create(
                ...
                data_retention_until=data_retention_until if data_retention_until else None,
            )
            messages.success(request, _('Patient added successfully! MRN: %(mrn)s') % {'mrn': mrn})
            return redirect('patients:patient_detail', pk=patient.pk)
        except Exception as e:
            messages.error(request, _('Error creating patient: %(error)s') % {'error': str(e)})
            return redirect('patients:add_patient')
