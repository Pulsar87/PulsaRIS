...
def get_hardware_id():
    ...
    try:
        if system == "Windows":
            ...
            board_id = open("/etc/machine-id").read().strip() if os.path.exists("/etc/machine-id") else "LINUX-BOARD"
        elif system == "Darwin":
            cmd = "ioreg -l | grep IOPlatformSerialNumber"
            board_id = subprocess.check_output(cmd, shell=True).decode().split('=')[-1].replace('"', '').strip()
    except:
    ...
    try:
        if system == "Windows":
            ...
            disk_id = subprocess.check_output("lsblk -no SERIAL /dev/sda", shell=True).decode().strip()
        elif system == "Darwin":
            cmd = "system_profiler SPStorageDataType | grep 'Serial Number' | head -n 1"
            disk_id = subprocess.check_output(cmd, shell=True).decode().split(':')[-1].strip()
    except:
